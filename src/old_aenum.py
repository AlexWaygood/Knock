...
class NamedTupleMeta(type):
	...
	def __new__(metacls, cls, bases, clsdict, size=undefined):
			...
			if k not in original_dict._field_names:
				...
				if isinstance(v, baseinteger):
					...
					if add_order:
						...
						target = unnumbered
				elif isinstance(v, tuple) and len(v) in (2, 3) and isinstance(v[0], baseinteger) and isinstance(v[1], (
				basestring, NoneType)):
		...
		assert set(fields) & set(offsets) == set(fields), "some fields are not in offsets: %s" % set(fields) & set(
			offsets)
		assert set(aliases) & set(offsets) == set(aliases), "some aliases are not in offsets: %s" % set(aliases) & set(
			offsets)
		for name, (index, doc, default) in offsets.items():
			...
			basestring, NoneType)) or doc is undefined, "doc is not a str, None, nor undefined (%s:%r)" % (
			name, type(doc), doc)
...
class EnumMeta(StdlibEnumMeta or type):
	...
	def __prepare__(metacls, cls, bases, init=None, start=None, settings=()):
		...
		for arg in settings:
			...
			allowed_settings[arg] = True
		enum_dict = _EnumDict(cls_name=cls, settings=settings, start=start, constructor_init=constructor_init,
